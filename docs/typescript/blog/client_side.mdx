...
description: "Building a fully client-side MCP debugging tool with zero backend dependencies—connecting directly to MCP servers from the browser while maintaining security and performance"
date: "2025-10-31"
---
...
Open `http://localhost:3000/inspector`, enter your MCP server URL, and start debugging—no installation, no backend, no proxy server. Everything runs in your browser.
...
export class BrowserMCPClient extends BaseMCPClient {
  protected createConnectorFromConfig(
    ...
    const { url, headers, authToken } = serverConfig;

    // Use HTTP connector with automatic SSE fallback
...
| ---------------- | ----------------------- | -------------------- |
| Transports       | stdio, SSE, HTTP        | HTTP, SSE            |
...
const [headers, setHeaders] = useState([{ name: "Authorization", value: "" }]);
...
const handleConnect = () => {
  ...
  addConnection(url, name, { headers: headerObj });
};
```
...
- [MCP Inspector Documentation](https://mcp-use.com/docs/inspector)
- [Authentication Guide](https://mcp-use.com/docs/typescript/server/authentication)
- [Building MCP Servers](https://mcp-use.com/docs/typescript/server)
