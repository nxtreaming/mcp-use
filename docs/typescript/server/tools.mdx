...
<Tip>
**Recommended Approach**: The recommended way to expose widgets to a model is to use the [widget helper in a custom tool](./ui-widgets#custom-tools-with-widgets) as shown above. Alternatively, you can set `exposeAsTool: true` in your widget's metadata to [auto-register it as a tool](./ui-widgets#automatic-widget-registration). The manual approach below is shown for reference.
</Tip>
