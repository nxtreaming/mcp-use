...
<Info>
**Widget Protocol Options**: This guide focuses on **ChatGPT Apps SDK** specifically. For the broader picture:
- [MCP Apps](/typescript/server/mcp-apps) - Standard MCP protocol with dual-protocol support (recommended)
- [ChatGPT Apps SDK](/typescript/server/ui-widgets) - This guide's focus
- [MCP UI](/typescript/server/mcp-ui-resources) - Alternative framework for simpler UIs

**Best Practice**: Use `type: "mcpApps"` to build widgets that work with **both** ChatGPT and MCP Apps clients from a single codebase.

</Info>

## Introduction

ChatGPT Apps enable developers to build rich, interactive user interfaces that go beyond simple text responses. Using the **ChatGPT Apps SDK**, you can create widgets that display dynamic content, handle user interactions, maintain state, and call tools—all within the ChatGPT interface.
...
- **Dual-protocol support** - widgets work with ChatGPT AND MCP Apps clients
- Built-in hooks that abstract protocol differences
...
1. **Single Command Setup**: `npx create-mcp-use-app my-app --template mcp-apps`
...
<img
  src="/images/inspector/chat-interaction.png"
  alt="Widget Execution in Inspector"
/>
...
<img
  src="/images/inspector/server-connected.png"
  alt="Server Connected Status"
/>
...
<CardGroup cols={2}>
  <Card
    title="UI Widgets Guide"
    icon="paintbrush"
    href="/typescript/server/ui-widgets"
  >
    Complete guide to building widgets with mcp-use, including advanced patterns
    and best practices
  </Card>
  <Card
    title="Creating Apps SDK Server"
    icon="server"
    href="/typescript/server/creating-mcp-apps-server"
  >
  ...
  <Card
    ...
    icon="puzzle-piece"
    href="/typescript/server/widget-components/mcpuseprovider"
  >
