...
class CustomServerManager {
  ...
  hasToolChanges(currentToolNames: Set<string>): boolean {
    ...
    return newToolNames.size !== currentToolNames.size
...
describe('server manager integration test', () => {
  it('should dynamically add tools through server manager', async () => {
    ...
    try {
      ...
      logger.info(`Initial server manager tools: ${serverManager.tools.map(t => t.name).join(', ')}`)

      expect(serverManager.tools.length).toBe(1)
      expect(serverManager.tools[0].name).toBe('get_greeting_tool')

      const query = 'Call get_greeting_tool to add the greeting tool, then use the greet tool to say hello to Alice'
      logger.info(`Query: ${query}`)

      const result = await agent.run(query)

      logger.info(`Result: ${result}`)
      logger.info(`Final server manager tools: ${serverManager.tools.map(t => t.name).join(', ')}`)
      logger.info('='.repeat(80) + '\n')
